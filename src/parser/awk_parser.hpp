...
class AwkParser : public Parser {
  ...
  virtual auto while_loop(const ParserFunc& t_func) -> NodePtr;
  virtual auto for_loop(const ParserFunc& t_func) -> NodePtr;
  virtual auto control(const ParserFunc& t_func) -> NodePtr;
